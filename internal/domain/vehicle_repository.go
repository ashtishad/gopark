...
func (v *VehicleRepositoryDB) ParkVehicle(ctx context.Context, plUUID uuid.UUID, regNum string) (*Vehicle, common.AppError) {
	...
	// find the numeric part from regNum, from ABC-123, extract 123
	numericPart := regexp.MustCompile(`\d+$`).FindString(regNum)
	vn, atoiErr := strconv.Atoi(numericPart)
	if atoiErr != nil {
		return nil, common.NewBadRequestError("unable to extract registration number trailing digit")
	}

	// this will be used to trigger different query
	isEven := vn%2 == 0
...
func (v *VehicleRepositoryDB) findNearestAvailableSlot(ctx context.Context, tx *sql.Tx, plID int, regNum string, isEven bool) (int, uuid.UUID, common.AppError) {
	...
	case err != nil:
		...
		return 0, uuid.Nil, common.NewInternalServerError(common.ErrUnexpectedDatabase, err)
	default:
		// msg="Chosen nearest slot available" "slot number"=1 vehicle=ABC-123
		v.l.Info("Chosen nearest slot available", "slot number", slotNum, "vehicle", regNum)
